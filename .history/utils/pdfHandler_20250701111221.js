...
import { bufferToStream } from './bufferToStream'; // utility to convert Buffer → Stream
...
export async function generatePDFAndSendEmail(userId, fullForm) {
  ...
  try {
    ...
    console.warn('Logo not found or invalid format:', err.message);
  }

  doc.moveDown(4);
  doc.fontSize(18).text('Shajid College Application Summary', {
    align: 'center',
  });
  doc.moveDown(2);
  ...
  // ✅ Helper functions
  const section = (title) => {
    doc.addPage(); // new section on a new page
